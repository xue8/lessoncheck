...
//                    }
    ...
    public void joinClassByLessonId(String lessonId, Handler h) {
        handler = h;
        User user = BmobUser.getCurrentUser(User.class);
...
//                            System.out.println("签到失败，已过签到时间");
                        ...
                        Log.d("测试", "插入成功" + list.get(0).getObjectId());
                    }else{
                        Message message = new Message();
                        message.obj = "课堂id不存在！";
                        message.what = 18;
    ...
    // 获取签到情况
    public void getSignStatusByLessonId(String lessonId, Handler h){
        handler = h;
        final User user = BmobUser.getCurrentUser(User.class);

        BmobQuery query =new BmobQuery("Lesson_Sign");
        query.addWhereEqualTo("lessonObjectId", lessonId);
        query.setLimit(30);
        query.findObjectsByTable(new QueryListener<JSONArray>() {
            @Override
            public void done(JSONArray ary, BmobException e) {
